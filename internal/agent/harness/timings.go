...
// run's outcome is decided, the orchestrator is still holding a concurrency slot
// and a claim (SPEC §9.3) until the process group is confirmed gone. It asks that
// of Probe while the record is still being written and of Stop once Done has
// closed — never of Done itself, which reports neither the group nor permission
// to reuse the workspace (#79). Teardown here is what these windows spend in the
// ordinary case, and it is:
//
...
//   - plus, for a process still alive past its terminal event, StopGrace before
//     reap intervenes and then two more graces for the SIGTERM→SIGKILL ladder.
//
...
type Timings struct {
	// StopGrace is how long SIGTERM has before SIGKILL follows (SPEC §7.5).
	// The ladder spends it twice in the worst case: once waiting out SIGTERM,
	// once confirming SIGKILL landed.
	...
	// GroupPoll is how often the signal ladder re-probes for the process
	// group's disappearance.
	GroupPoll time.Duration
