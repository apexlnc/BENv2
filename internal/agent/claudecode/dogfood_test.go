...
// the provider block is opaque to the loader, so only the adapter can say
// whether the dogfood config would actually dispatch. `make workflow-check` now
// asks the same question through the CLI (#55); this keeps the answer next to
// the adapter, where a newly required provider key is added (AGENTS.md,
// Dogfooding).
func TestDogfoodWorkflowProviderBlock(t *testing.T) {
	...
	if def.Config.Agent.Kind != claudecode.KindName {
	...
	var kind core.RunnerKind = claudecode.Kind{}
